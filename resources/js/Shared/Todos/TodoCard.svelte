<script>
    ...
    import TodoCardActions from "./TodoCardActions.svelte";
...
<div class="card bg-base-200 shadow-xl">
    <div class="card-body min-h-[90vh] max-h-[90vh] pb-28 relative">
        <div class="flex justify-between items-center">
            ...
            <TodoCardActions isListInGroup={list.parent_id} canDeleteCompleteTodos={todos_done.length > 0} on:editListTitle={() => editTitleInputEnable = true} />
        </div>
        <div class="divider"></div>
        {#if !todos.length && !todos_done.length}
        ...
        {:else}
            <div class="overflow-y-auto">
                ...
                {#if showCompletedTodos}
                    <ul>
                        <SortableList animation="500" onSort={sortList}>
                            {#each todos_done as item (item.id)}
                                <li data-id="{item.id}">
                                    <TodoItem todo={item} active={$todoDetail.showCard && (item.id === $todoDetail.todo.id)} />
                                </li>
                            {/each}
        ...
        <audio class="hidden" id="completeTodoSound" src="{route('home')}/sounds/ding.mp3">
